 ...
 * Random create food
 */
public class PoisonController implements Runnable, EventListener {

    private final JLabel[][] labels;
    private final Snake snake;
    private volatile boolean running = true;

    public PoisonController(JLabel[][] labels, Snake snake) {
        this.labels = labels;
        this.snake = snake;
    }
    ...
    public void run() {
        while(true) {
            if (running) {
                ...
                labels[randRow][randCol].setBackground(SnakeEnum.POISON.getColor());
                try {
                    Thread.sleep(1_000);
